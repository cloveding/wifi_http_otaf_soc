...
#define BMPIMAGEOFFSET 66 // The BMP image header size is 66 bytes
...
void setup() {
  ...
  myCAM.InitCAM();
  myCAM.OV2640_set_JPEG_size(OV2640_320x240); // default size: 320x240
  delay(1000);
...
void loop() {
  ...
  if (Serial.available())
  {
    ...
    temp = 0xff;
      ...
      case 0x10:
      mode = 1; // mode 1: single frame capture
      temp = 0xff;
      start_capture = 1;
      ...
      case 0x30:
      mode = 3; // mode 3: single frame BMP capture
      temp = 0xff;
      start_capture = 3;
      Serial.println(F("ACK CMD CAM start single shoot. END"));
      break;
  ...
  if (mode == 1) // single frame capture
  {
    if (start_capture == 1)
    {
      myCAM.flush_fifo();
  ...
  else if (mode == 2) // video streaming
  {
    while (1)
    {
      temp = Serial.read();
  ...
  {
    if (start_capture == 3) // single frame BMP capture
    {
      //Flush the FIFO
      myCAM.flush_fifo();
      myCAM.clear_fifo_flag();
