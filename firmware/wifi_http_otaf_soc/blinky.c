...
static sl_si91x_gpio_pin_config_t sl_gpio_pin_config = { { SL_SI91X_ULP_GPIO_1_PORT, GPIO_PIN_NUMBER1 }, GPIO_OUTPUT };
/*******************************************************************************
...
void gpio_detailed_example_init(void)
{
    ...
    sl_gpio_set_pin_mode(sl_gpio_pin_config.port_pin.port, sl_gpio_pin_config.port_pin.pin, SL_GPIO_MODE_0, 0);
    sl_gpio_set_configuration(sl_gpio_pin_config);

    // sl_gpio_set_pin_output(sl_gpio_pin_config.port_pin.port, sl_gpio_pin_config.port_pin.pin);
}
/*******************************************************************************
...
void gpio_detailed_example_process_action(void)
{
  ...
  sl_gpio_driver_toggle_pin(&sl_gpio_pin_config.port_pin); // Toggle HP GPIO pin 1
...
static void delay(uint32_t idelay)
{
  for (volatile uint32_t x = 0; x < MS_DELAY_COUNTER * idelay; x++) {
    __NOP();
  }
}
